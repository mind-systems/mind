...
`deploy-staging.sh` at the repo root handles the full cycle for a given service:

```bash
# Deploy API
./deploy-staging.sh mind_api

# Deploy web
./deploy-staging.sh mind_web
```
...
3. Copies `docker-compose.staging.yml` and env files to `/srv/mind/`
...
- `.env.staging` at repo root (`VITE_API_BASE_URL`)
- `mind_api/.env.staging` with all API + DB configuration

## Env files

| File | Used by | Scope |
|------|---------|-------|
| `.env.staging` | root compose (build args) | `VITE_API_BASE_URL` for web build |
| `mind_api/.env.staging` | postgres + mind_api containers | DB credentials, JWT, ports, gRPC, CORS |

`mind_api/.env.staging` is the single source of truth for the API and database config. The root compose reads it directly via `env_file: ./mind_api/.env.staging`.
